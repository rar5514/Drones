globals
[
target-hits cohesion-flag normal-flag casualties energy speed
]
breed [flowers flower]
breed [houses house]
breed [concertinas concertina]
breed [EODs soldier]
breed [flags flag]
breed [cars car] ;;vehicles on road of which are candidates to be VBIEDs based on user settings
breed [Ecars ecar] ;;enemy cars (static)
breed [airplanes airplane]

EODs-own[ visible-neighbors flockmates nearest-neighbor     ;; what birds can I see nearby?
             closest-neighbor]
to setup
  clear-all
  setup-patches

  ;;Creates houses in hard coded locations
  create-houses 8 [set shape "house" set size 1 set color brown]
  ask house 0
  [setxy 3 21]
  ask house 1
  [setxy 23 -4]
  ...
  ;;Creates the rst of the environment upon clicking the Setup button
  create-houses 1 [set shape "courtyard1" set size 4 setxy 21 21 facexy 0 0 ] ;; house with courtyards
  create-houses 1 [set shape "courtyard1" set size 4 setxy 18 24 facexy 0 0 ] ;; house with courtyards
  create-houses 1 [set color blue set size 3 set shape "house two story" setxy -7 -26]
  create-houses 1 [set color blue set size 3 set shape "house two story" setxy 0 -26]
  create-houses 1 [set color blue set size 3 set shape "house two story" setxy 7 -26]
  create-concertinas 1 [set shape "concertina" set color black setxy -3 -20 set size 8 facexy -3 0]
  create-concertinas 1 [set shape "concertina" set color black setxy -9 -26 set size 8 facexy -11 -26] ;;BL
  create-concertinas 1 [set shape "concertina" set color black setxy 5 -20 set size 8 facexy 6 0]
  create-concertinas 1 [set shape "concertina" set color black setxy 11 -27 set size 8 facexy 26 -26]  ;;BR
  create-flags 1[set shape "flag" set color blue setxy 1 -22 set size 4]
  create-Ecars 1 [set shape "truck"  set color 63 setxy -7 19 set size 1]
  create-Ecars 1 [set shape "ambulance" set color 63 setxy -8 21 set size 1.5]
  create-Ecars 1 [set shape "car"  set color 63 setxy -10 20 set size 1]
  ;ask n-of explosives patches with [pycor > 0][set pcolor red] ;; no longer needed since the explosives are now static
  create-flowers 50 [setxy random-xcor random-ycor set shape "flower budding" set color 3]

  ;;creates drone and their location as well as other characteristics
  create-airplanes Drones [ set size 1 set shape "ufo side" facexy 20 21 set energy initial-energy
      ; setxy 0 -20
    set color blue
      ;setxy 0 75
      rt random-float 360
  ...
  ;;EOD settings and characteristics Team 1
  create-EODs EOD [ set size 1 set shape "person soldier"  set energy initial-energy
      if first search-pattern != "D"
        [ set color 105 ]
      setxy -20 6
      rt random-float 360
      set flockmates no-turtles ]

  ;;EOD settings and characteristics Team 2
  create-EODs EOD [ set size 1 set shape "person soldier"  set energy initial-energy
      if first search-pattern != "D"
        [ set color 105 ]
      setxy 23 6
  ...
  setup-cars
  set target-hits 0
  set energy initial-energy / 1000
  set casualties 0
  reset-ticks
...
to setup-cars
  ...
  create-cars number-of-cars [
   ...
   ask n-of enemy cars [set color red]
end
to setup-patches
  ifelse (first target-pattern = "S")
  [
  ask patches [ set pcolor 37 ]
  ask patches with [abs pycor < 1] [ set pcolor 9]  ;; setup-road

  ;;Scattered target-pattern option
  ask patches with [pxcor = -32 and pycor = 22][set pcolor red]
  ask patches with [pxcor = -42 and pycor = 13][set pcolor red]
  ask patches with [pxcor = 1 and pycor = 27][set pcolor red]
  ask patches with [pxcor = 6 and pycor = 5][set pcolor red]
  ask patches with [pxcor = 18 and pycor = 18][set pcolor red]
  ask patches with [pxcor = 35 and pycor = 7][set pcolor red]
  ask patches with [pxcor = 26 and pycor = 25][set pcolor red]
  ask patches with [pxcor = 39 and pycor = 18][set pcolor red]
  ask patches with [pxcor = -28 and pycor = 6][set pcolor red]
  ask patches with [pxcor = -12 and pycor = 12][set pcolor red]
  ask patches with [pxcor = 26 and pycor = 14][set pcolor red]
  ask patches with [pxcor = -28 and pycor = 14][set pcolor red]
  ]
  [
  ask patches [ set pcolor 37 ]
  ask patches with [abs pycor < 1] [ set pcolor 9]  ;; setup-road

  ;;Linear target-pattern option
  ask patches with [pxcor = -45 and pycor = 1][set pcolor red]
  ask patches with [pxcor = -41 and pycor = 3][set pcolor red]
  ask patches with [pxcor = -37 and pycor = 5][set pcolor red]
  ask patches with [pxcor = -33 and pycor = 7][set pcolor red]
  ask patches with [pxcor = -29 and pycor = 9][set pcolor red]
  ask patches with [pxcor = -25 and pycor = 11][set pcolor red]
  ask patches with [pxcor = -21 and pycor = 13][set pcolor red]
  ask patches with [pxcor = -17 and pycor = 15][set pcolor red]
  ask patches with [pxcor = -13 and pycor = 17][set pcolor red]
  ask patches with [pxcor = -9 and pycor = 19][set pcolor red]
  ask patches with [pxcor = -5 and pycor = 21][set pcolor red]
  ask patches with [pxcor = -1 and pycor = 23][set pcolor red]
  ]
end
to go
  ;if initial-energy = 0 [stop]
  if (first search-pattern = "1")
  [ask airplanes
    [ forward 4
      ifelse ( ycor < -10 ) [ facexy random-xcor 150 ]
    [ rt random 270
        lt random 270 ]
     if not can-move? 1 [rt 180]
    set initial-energy (initial-energy - 4)
    if (ycor < -15 ) [ facexy random-xcor 150 ]
     sense ]]
  if (first search-pattern = "2")
  [ ask airplanes [
    ifelse random 100 < 20
    [ forward 10
     rt random 270
     lt random 270
    set initial-energy (initial-energy - 10)
    ]
    [forward 4
     ; rt random 360
      ;lt random 360
  ...
  ask EODs [
    ...
    lt random 10
    if not can-move? 1[rt 180]
    if (ycor < 10 ) [ facexy random-xcor 150 ]
    ; set initial-energy (initial-energy - 1)
    if pcolor = red [set pcolor 37 set casualties casualties + 1]
  ]
  ask EODs
        [ flock
...
tick
  set energy initial-energy / 1000
  check-death
  output-write (word "Tick Number: " ticks "----")
  output-write (word "Energy: " energy "----")
  output-write (word "Enemy Detected: " target-hits "--")
  output-show (word "EOD casualties: " casualties)
  if initial-energy <= 0 [ stop ]
end

;;Snipetted adopted from Flock model used for EOD to manupulate their movement settings
to flock  ;; turtle procedure
  find-flockmates
  if any? flockmates
    [ find-nearest-neighbor
      ifelse distance nearest-neighbor < 6.00
        [ separate ]
        [ align
          cohere ] ]
  fd .1
...
to flock-mode  ;; turtle procedure
  find-flockmates
  if any? flockmates
    [ find-nearest-neighbor
      ifelse distance nearest-neighbor < 6.00
...
to flock-shape-mode  ;; turtle procedure
  find-flockmates
  if any? flockmates
    [ find-nearest-neighbor
      ifelse distance nearest-neighbor < 6.00
...
to find-flockmates  ;; turtle procedure
  set flockmates other EODs in-radius 10.00
end
...
to separate  ;; turtle procedure
  turn-away ([heading] of nearest-neighbor) 15.25
end

;;; ALIGN

to align  ;; turtle procedure
  turn-towards average-flockmate-heading 10.00
end
...
to cohere  ;; turtle procedure
  turn-towards average-heading-towards-flockmates 10.00
end
...
to check-death
  ask airplanes
  [if initial-energy <= 0 [ die  ] ;; removes the turtle if it has no energy left
  ]
  ask EODs [
    if pcolor = red [set pcolor 37 set casualties casualties + 1]
    ;if pcolor = red [set pcolor 37 set casualties casualties + 1]
...
to sense
  ask patches in-radius sense-radius
  [
    if pcolor = red
    [ set target-hits target-hits + ( count patches in-radius sense-radius with [pcolor = red] )
      ask patches in-radius sense-radius [ if pcolor = red [ set pcolor 37 ] ] ] ; new addition to make sensed IEDs within range turn tan
  ]
  ask patches in-radius sense-radius
  [
    if ( any? cars-here with [color = red] )
       [
          set target-hits target-hits +  1
          ; ( count cars in-radius sense-radius with [color = red] )
         ask patches in-radius 2 [ set pcolor yellow ]
         ask patches in-radius 1 [ set pcolor 124 ]
         ask cars-here [ die ]
    ]
     if ( any? Ecars-here with [color = red] )
       [
          set target-hits target-hits +  1
          ; ( count cars in-radius sense-radius with [color = red] )
         ask patches in-radius 2 [ set pcolor yellow ]
  ...
  ask EODs[
  if pcolor = red [set pcolor 37 set casualties casualties + 1 set target-hits target-hits +  1]
  ]
end
 ; reserve code below, this procedure was for sensing only by the patch here, ie: patch the drone is actually on
 ; if pcolor = red [set pcolor 37 set target-hits target-hits + 1 ]
     ;if ( any? cars-here with [color = red1] )
      ;[
       ;ask patches in-radius 2 [ set pcolor yellow ]
        ;ask patches in-radius 1 [ set pcolor 124 ]
        ;ask cars-here [ die ]
        ;set target-hits target-hits + 1
       ; set initial-energy initial-energy + 1000
      ;]
     ; if ( any? Ecars-here with [color = red ] )
      ; [
       ;  ask patches in-radius 2 [ set pcolor yellow ]
...
search-pattern
search-pattern
"1 Random Pattern" "2 Natural Hunter (Levy)"
0
...
1892
420
Energy - Drone Target Detection - Casualties
Time (ticks)
NIL
0.0
10.0
...
"" ""
PENS
"Explosives Detected" 1.0 0 -2674135 true "" "plot target-hits"
"Drone Energy" 1.0 0 -14439633 true "" "plot energy"
"EOD Casualties" 1.0 0 -13628663 true "" "plot casualties"
...
This model attempts to simulate a desert environment with improvised explosive devices (IED's) and Vehicle Bourne Improvised Explosive Devices (VBIEDs). The IED's are statically placed North of a road which is frequented by VBIED's and general vehicular traffic.  The southern part of the road has been cleared by friendly forces and a Forward Operating Base (FOB) has been established.  The layout further consists of of living structures featuring courtyards, abandoned vehicles, and other living structures  which are a common feature in these areas. 

## HOW IT WORKS

The agents in the model follow a specific set of instructions as configured by the user.
-The Troops on the ground aka (Explosive Ordenance Detachment; EOD) survey the area and clear IED's as they encounter them.  They work in teams and have a preset area of operation (East and West respectively).  EOD is limited to clearing IED's and static vehicles. 
-Drones have free range of the field with their primary target consisting of clearing VBIEd's.  Drones are equipped to trigger explosives therefore, once a VBIED is triggered, a splash is created on the location which can be utilized for battle tracking purposes, intelligence and to track enemy Tactctics Techniques and Procedures (TTP's).  Drones Conduct reconnaisance in 1 of 2 formations as per user preference and target pattern selection.


## HOW TO USE IT

The user can run an array of simulations given the various setting options.  The model supports the simultaneous activities of both Drones and EOD however, they may be run independantly base on mission reuqirements.  

Setup:  Set the environment as per the user settings.
Go: Runs the simulation.

Sliders:

Drones: max 5
Sense Radius: max 60 Gives drones a field of vision
Step:  max 1.00 speeds and slows drones
Initial-energy:  max 20,000 Drone and EOD share the same energy.  May be adjusted for shorter simulations.  When energy = 0, the simulation stops.
Search-Pattern:  2 options to choose from.  Used to test the efficiency of clearing the area based on enemy composition and disposition.
EOD: max 9 may be adjusted for specific mission requirements.  1 team in the east 1 in the west.
Number-of-cars: max 20. set at min 10 to prevent system error from VBIED settings and to determine traffic density
Enemy (VBIED): max 10. Used in conjunction with Number-of-cars.

*Intelligence Report*
Monitors the total number of enemy in Area of Operation (AO).  Counts down to show when cleared.

*Battle Damage Assessment (BDA)*
Monitors and tracks enemy disposition as interacted by Drones and EOD.  Counts up to denote actions by agent group.


## THINGS TO NOTICE

Survey your landscape to determine the number of friendly's required to execute the required mission (simulation).  Observe drone movement to determine what formation works best for the type of enemy situation.  Notice the 3 static vehicles as random IED's may present themselves.  Detonated IED's are removed and a splash is created in the impact area.

## THINGS TO TRY
1.  Slow down the model to get a closer view of the action. 
2.  Run simulation with Drones only to test their efficiency.
3.  Run EOD only 
4.  Increase Sense-radius to increase clearing distance (does it work with drones and EOD?)
5.  Using shape editor, create other shapes that may enhance the layout of the land (as we did with the courtyards).


## EXTENDING THE MODEL

Try the following:
1.  Add a random VBIED and give it free range of the land.  Have it detonate when it comes into contact with ground forces.
2.  Setup an IED to be uncleared and detonate on contact from EOD.  
3.  Add a morale component to EOD.  Decrease when friendly forces are lost and increased when IED is cleared.
4.  Set a random building red for clearing by IED 
5.  Link EOD to clear IED's together and move within 1 patch of each other.


## RELATED MODELS
-Flocking (EOD movement)
-Traffic Basic
...
Wilensky, U. (1998). NetLogo Basic Traffic model. http://ccl.northwestern.edu/netlogo/models/Flocking. Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
@#$#@#$#@
default
true
0
...
true
0
Circle -7500403 false true 0 120 30
Circle -7500403 false true 15 105 30
Circle -7500403 false true 30 90 30
Circle -7500403 false true 45 75 30
Circle -7500403 false true 60 75 30
Circle -7500403 false true 75 75 30
Circle -7500403 false true 90 75 30
Circle -7500403 false true 105 75 30
Circle -7500403 false true 120 75 30
Circle -7500403 false true 135 75 30
Circle -7500403 false true 150 75 30
Circle -7500403 false true 165 75 30
Circle -7500403 false true 180 75 30
Circle -7500403 false true 195 75 30
Circle -7500403 false true 210 75 30
Circle -7500403 false true 225 90 30
Circle -7500403 false true 240 105 30
Circle -7500403 false true 255 120 30
...
true
0
Rectangle -6459832 true false 45 60 255 90
Rectangle -6459832 true false 45 90 75 150
Rectangle -6459832 true false 225 90 255 150
Rectangle -6459832 false false 60 60 90 75

courtyard1
true
0
Rectangle -6459832 true false 45 45 255 105
Rectangle -6459832 true false 45 105 90 135
Rectangle -6459832 true false 210 105 255 135
...
true
9
Polygon -13791810 false true 45 75 45 225 285 225 210 150 285 75 45 75 45 75 45 75
...
@#$#@#$#@
<experiments>
  <experiment name="Scattered-random" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>target-hits + casualties = 12</exitCondition>
    <metric>ticks</metric>
    <metric>initial-energy</metric>
    <metric>target-hits</metric>
    <metric>casualties</metric>
    <enumeratedValueSet variable="EOD">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Sense-radius">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Enemy">
      <value value="0"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="search-pattern">
      <value value="&quot;1 Random Pattern&quot;"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="target-pattern">
      <value value="&quot;Scattered&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Scattered Levy" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>target-hits + casualties = 12</exitCondition>
    <metric>ticks</metric>
    ...
    <enumeratedValueSet variable="search-pattern">
      <value value="&quot;2 Natural Hunter (Levy)&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="step">
      <value value="0.08"/>
    </enumeratedValueSet>
  ...
  <experiment name="Linear-Levy" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>target-hits + casualties = 12</exitCondition>
    <metric>ticks</metric>
  ...
  <experiment name="Linear-Random" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>target-hits + casualties = 12</exitCondition>
    <metric>ticks</metric>
  ...
  <experiment name="Scattered-random-5vbied" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>target-hits + casualties = 17</exitCondition>
    <metric>ticks</metric>
    <metric>initial-energy</metric>
    <metric>target-hits</metric>
    <metric>casualties</metric>
  ...
  <experiment name="Scattered-levy-5vbied" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>target-hits + casualties = 17</exitCondition>
    <metric>ticks</metric>
